  ...
  for (const bookSlug of bookSlugs) {
    ...
    for (const chapterInfo of outline.chapters) {
      const chapterFilePath = path.join(
        bookDir,
        `${String(chapterInfo.chapter).padStart(2, "0")}-${slugify(
          chapterInfo.title
        )}.md`
      );
      if (!fs.existsSync(chapterFilePath)) {
        const previousChapterSummary =
          chapterInfo.chapter > 1
            ? outline.chapters[chapterInfo.chapter - 2].summary
            : null;
        return {
          bookSlug,
          bookDir,
...
async function startOfflineWorker() {
  console.log(
    "🚀 Offline AI Worker đã khởi động (State được quản lý bởi Firestore)."
  );
  ...
  while (true) {
    try {
      ...
      if (!taskInfo) {
        console.log(
          `(Offline) Không tìm thấy chương nào để viết ở local. Kiểm tra lại sau ${CHECK_INTERVAL_MINUTES} phút...`
        );
      ...
      if (!canWork) {
        console.log(
          `(Offline) [${taskInfo.bookSlug}] Không thể đòi quyền viết. Worker khác có thể đang làm việc hoặc đã hết lượt.`
        );
      ...
      if (fs.existsSync(worldFilePath)) {
        try {
          const worldBibleJSON = JSON.parse(
            fs.readFileSync(worldFilePath, "utf-8")
          );
          worldBibleContent = formatWorldBibleForPrompt(worldBibleJSON);
        } catch (e) {
          worldBibleContent = fs.readFileSync(worldFilePath, "utf-8");
        }
      }
      console.log(
