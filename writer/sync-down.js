...
if (!admin.apps.length) {
  if (!FIREBASE_SERVICE_ACCOUNT) {
    throw new Error(
      "Không thể khởi tạo sync-down.js do thiếu cấu hình Firebase."
    );
  }
  admin.initializeApp({
...
async function syncDownFromFirebase() {
  ...
  for (const bookDoc of booksSnapshot.docs) {
    ...
    // 3. Tạo file world.json nếu chưa có
    const worldFilePath = path.join(bookDir, "world.json");
    if (!fs.existsSync(worldFilePath) && bookData.worldBible) {
      fs.writeFileSync(worldFilePath, bookData.worldBible);
      console.log(`  -> Created missing local world.json.`);
    }

    // 4. Lấy tất cả các chương đã viết từ sub-collection trên Firestore
    ...
    for (const chapterDoc of chaptersSnapshot.docs) {
      ...
      const chapterFileName = `${String(chapterData.chapterNumber).padStart(
        2,
        "0"
      )}-${slugify(chapterData.title)}.md`;
