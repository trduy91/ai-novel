...
app.get('/book/:slug/:chapterFile', async (req, res) => {
    ...
    if (fs.existsSync(localChapterPath)) {
        ...
        try {
            ...
            const chapterDoc = await db.collection('books').doc(slug).collection('chapters').doc(chapterId).get();
            if (chapterDoc.exists) {
                markdownContent = chapterDoc.data().content;
                source = 'từ Firebase';
            } else {
                return res.status(404).send("Không tìm thấy chương này ở cả cục bộ và Firebase.");
            }
        } catch(e) {
    ...
    let prevChapterLink = null, nextChapterLink = null;
    const chaptersSnapshot = await db.collection('books').doc(slug).collection('chapters').orderBy('chapterNumber').get();
    ...
    if (currentIndex > 0) {
        ...
        prevChapterLink = `/book/${slug}/${String(prev.chapterNumber).padStart(2, '0')}-${slugify(prev.title)}.md`;
    }
    if (currentIndex < allChapters.length - 1) {
        const next = allChapters[currentIndex+1];
        nextChapterLink = `/book/${slug}/${String(next.chapterNumber).padStart(2, '0')}-${slugify(next.title)}.md`;
    }
...
app.listen(port, () => {
    console.log(`📚 Publisher (Hybrid Mode) đang chạy tại http://localhost:${port}`);
});